...
3. 【强制】类名使用`UpperCamelCase`风格, 数据传输对象以Dto结尾，ice定义对象以MIce前缀开始。 
...
16. 【参考】各层命名规约：  
  A) Service<br>
   ...
   4） 新增实体的方法用save/create作前缀。    
   5） 删除的方法用remove/delete作前缀。    
   6） 修改实体的方法用save/update作前缀。 
  <br>
  B) Mapper<br>
  	1) 获取单个对象用select做前缀。
  	<br>2) 获取列表时，条件简单时，用select做前缀；当条件复杂，涉及多表查询时，使用search前缀。
  	<br>3) 获取统计值时用count作前缀。
  	<br>4) 新增用insert做前缀。
  	<br>5) 删除用delete做前缀。
  	<br>6) 更新用update做前缀。
  	<br>
