...
9. 【强制】表必备三字段：code, created_at, created_by。当表有更新时，需要有updated_at，updated_by；当记录是软删除时，需要有is_deleted；当表有乐观锁时，使用version；预计该表数据量不会特别大，不会被分库分表的前提下，可以使用数据库自增主键id，而不使用code；
