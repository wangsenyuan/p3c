...
9. 【参考】`@Transactional`事务不要滥用。事务会影响数据库的QPS，另外使用事务的地方需要考虑各方面的回滚方案，包括缓存回滚、搜索引擎回滚、消息补偿、统计修正等。 `@Transactional` 只能添加在方法上，当该事务不涉及更新操作时，标明`readOnly=true`；在不确定不会引起死锁的情况下，不适用`progation=REQUIRES_NEW`； 
